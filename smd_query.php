...
: Default: @6:5@.
; %(atnm)silent% : if your query contains an error (wrong column name or some malformed input), the plugin will issue a Txp or SQL error message. Using @silent="1"@ will attempt to hide this error message.
; %(atnm)var_prefix%
...
h2(atts #replacements). Replacement tags
...
* %(atnm){id}% : the image ID
* %(atnm){name}% : the image filename
* %(atnm){category}% : the image category
...
* %(atnm){smd_allrows}% : the total number of rows in this result set[1]
* %(atnm){smd_pages}% : the total number of pages in this result set[1]
* %(atnm){smd_thispage}% : the current page number being viewed[1]
* %(atnm){smd_rows}% : the total number of rows visible on this page
* %(atnm){smd_thisrow}% : the current row number on this page
* %(atnm){smd_thisindex}% : the current row number (zero-based) on this page
* %(atnm){smd_cursor}% : the current row number from the start of the result set
* %(atnm){smd_cursorindex}% : the current row number from the start of the result set (zero-based)
* %(atnm){smd_usedrow}% : the current row number of rows that have actually been displayed. If your smd_query container employed a conditional, for example, that only output certain rows then this counter only increments for those rows you display. Bear in mind that using conditionals inside smd_query can mess paging and other counters up, so it's best to avoid paging if you are using conditional output.

fn1. These items are also available in your designated @pageform@. The pageform can also utilise these extra replacements:

* %(atnm){smd_prevpage}% : the previous page number (empty if on first page)
* %(atnm){smd_nextpage}% : the next page number (empty if on last page)
* %(atnm){smd_row_start}% : the first row number being displayed
* %(atnm){smd_row_end}% : the last row number being displayed
* %(atnm){smd_rows_prev}% : the number of rows on the previous page. Will either be the value of your @limit@, or 0 if on the first page
* %(atnm){smd_rows_next}% : the number of rows on the next page
* %(atnm){smd_query_unique_id}% : the unique ID assigned to this smd_query tag (see the @hashsize@ attribute and "example 12":#eg12 for more)
...
bc(block). <txp:smd_query_if_prev>Previous page</txp:smd_query_if_prev>
...
bc(block). <a href="/images/{id}{ext}" /><txp:thumbnail name="{name}" /></a>
...
bc(block). <txp:smd_query query="SELECT DISTINCT
...
bc(block). <txp:category name="{name}" link="1" title="1" />
...
bc(block). <txp:category name="{name}" link="1" title="1" /> ({count})
...
bc(block). <txp:smd_query column="*" table="txp_file"
...
bc(block). <txp:smd_query query="SELECT *
...
bc(block). <txp:variable name="cutoff"
     value="42" />
...
bc(block). <txp:smd_query query="SELECT Title
     FROM textpattern
...
bc(block). <txp:smd_query query="SELECT * FROM txp_users"
...
bc(block). <txp:output_form form="user_table">
<td>{name}</td>
<td>{RealName}</td>
</txp:output_form>
...
bc(block). Page {smd_thispage} of {smd_pages} |
...
bc(block). ==<txp:smd_query query="SELECT * FROM txp_users WHERE
